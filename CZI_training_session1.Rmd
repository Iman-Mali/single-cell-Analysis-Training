---
title: "CZI single-cell data analysis training series: Session 1"
...
sci-help@brotmanbaty.org

#################################################################################################
- Office hour support
- R, Rstudio, Monocle 3 and Garnett installed
#################################################################################################
...
These questions are difficult! You do not have all the information you need provided to you - you'll need
to find further information on the internet. Attempt these if you're ahead in the tutorial exercises or in your free time.
#################################################################################################
...
## Today we will read in a pre-made CDS for running through our session code
```{r echo=TRUE}
#Here I'm reading in a pre-made cds object for a sample named Sentinel (provided to you as well):
cds <- readRDS('./Sentinel_cds.RDS')

```

# If you need to combine multiple cds objects, use the combine_cds function
```{r echo=TRUE}
#cds_combined <- combine_cds(list(cds1=cds1, cds2=cds2))
```
...
## IF YOU DIDN'T HAVE A PRE-MADE CDS YOU COULD LOAD IT USING THE FOLLOWING 2 WAYS:

```{r load, echo=FALSE}
# 1. From an online storage location like a database:
#Function 1: new_cell_data_set
...
# 2. Using the counts matrix, gene annotations and cell annotations files:
...
#Here, you can read in the UMI counts matrix and the gene & cell annotations to make a brand new cds object:

#my_new_cds <- load_mm_data(mat_path = "./umi_counts.mtx", feature_anno_path = "./gene_annotations.txt", cell_anno_path = "./cell_annotations.txt", umi_cutoff = 100, feature_metadata_column_names=c('gene_short_name'), sep="")

# Where:
# umi_cutoff - UMI per cell cutoff. Columns (cells) with less than umi_cutoff total counts are removed from the matrix. Default = 100.
...
#################################################################################################
######################EXTRA NOTE: For users working with 10X data!###############################
#################################################################################################
## How to load 10X data in Monocle 3?
...
```{r echo=FALSE}
# As you can see, there is already a n.umi column in the colData(cds), with a default cut off of 100.
...
# You can further filter for unreasonably high UMI counts (could often indicate doublets or leaking)
# Hint: Look at summary(colData(cds)$n.umi) and decide your cut-offs
# For eg:
#cds <- cds[,colData(cds)$n.umi < 50000]


## B. DEAL WITH HIGH MITOCHONDRIAL READS %
...
# Several different doublet callers - DoubletFinder, scrublet, etc.
